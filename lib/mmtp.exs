...
#  IO.inspect result
 ...
 def search(city) do
  ...
  search_result_temp = result
   ...
   |> Enum.at(0)
   |> Enum.at(4)
 IO.inspect search_result_temp
  if(search_result_temp && search_result_temp != "") do
...
#  IO.inspect data_path 
 ...
 def geocode(station) do
  ...
  url_name = String.replace(name, " ", "%20")
#15
  url = "https://maps.googleapis.com/maps/api/geocode/json?address=#{url_name}%20#{code},India&key=AIzaSyCgEUBBK2jh3eMcT7RajrxNolcmpZBiSmQ"
  case HTTPoison.get(url) do
   {:ok, %HTTPoison.Response{status_code: 200, body: body}} ->
     {:ok, address} = body |> String.replace("\n", "") |> JSON.decode
