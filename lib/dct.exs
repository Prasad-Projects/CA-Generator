defmodule Dct do 
 def dct_txt do
   ...
   IO.puts "Running DCT script (This may take a few minutes) ........"
   station_ids
   |> Enum.map(fn {code, id} -> :ets.insert(:station_ids, {String.strip(code), id}) end)
   sch = fetch_train_days
   result = result
   |> Map.get(:rows)
   |> Enum.map(fn [x] -> Task.async(fn -> get_c(x, sch, p) end) end)
   |> Task.yield_many(9999999)
   IO.puts "Done"
 end

 def dct_update(train_id) do
   {:ok, p} = Mariaex.Connection.start_link(username: "sushruth456", password: "sushruth456", database: "mmtp")
   station_ids = fetch_station_ids
   :ets.new(:station_ids, [:public, :named_table])
   IO.puts "Updating for train #{train_id} ..."
   station_ids
   |> Enum.map(fn {code, id} -> :ets.insert(:station_ids, {String.strip(code), id}) end)
   sch = fetch_train_days
   get_c(train_id, sch, p)
 end
...
if main_argv == "update" do
  Dct.update_train Enum.at(System.argv, 1)
else
  Dct.dct_txt
end
